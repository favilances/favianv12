module.exports = {
   ...
   async favian(client, message, args) {
    ...
    let yetkili = db.fetch(`sunucu.ayarlar.kicksistem.yetkili_${message.guild.id}`)
    const sembed = new Discord.MessageEmbed()
    .setColor(client.ayarlar.embedRenk)
    .setAuthor(`${client.user.username} » At`, client.user.displayAvatarURL({dynamic: true}))
    ...
    .setFooter(client.ayarlar.embedFooter)
        if(!yetkili) return message.inlineReply(sembed);
            ...
            let log = db.fetch(`sunucu.ayarlar.kicksistem.log_${message.guild.id}`)
    ...
    .setDescription(`\`${banuser.tag}\`  **Adlı şahsın Atılmasını onaylıyor musunuz?**`)
    .setColor(client.ayarlar.embedRenk)
    .setFooter(client.ayarlar.embedFooter)
    .setAuthor(`${client.user.username} » At`, client.user.displayAvatarURL({dynamic: true}))
    let msg = await message.channel.send({ embed: onayy , buttons : [ button, button2]})
      ...
      if(button.id === "banonay") {
        ...
        const okembed = new Discord.MessageEmbed()
                .setColor(client.ayarlar.embedRenk)
                .setAuthor(`${client.user.username} » At`, client.user.displayAvatarURL({dynamic: true}))
                .setDescription(`\<:savana_banhammer:929104901584334988>   | » ${banuser} başarıyla atıldı gerekli bilgiler <#${log}> kanalına gönderildi!`)
                .setFooter(client.ayarlar.embedFooter)
