...
module.exports = {
   ...
   async favian(client, message, args) {
    ...
    const embed = new Discord.MessageEmbed()
          ...
          .setAuthor(`${client.user.username} » Ban Ayarla`, client.user.displayAvatarURL({dynamic: true}))
  ...
  if (args[0] == 'yetkili') {
    let rol = message.mentions.roles.first();
           ...
           .setTitle(`${client.user.username} - Ban Yetkili Rol Ayarla `)
           .setColor(client.ayarlar.embedRenk)
         ...
         .setTitle(`${client.user.username} - Ban Yetkili Ayarlandı `)
         .setColor(client.ayarlar.embedRenk)
         .setDescription(`Ban Yetkili Rol Başarıyla ${rol} Olarak Ayarlandı ! `)
         .setThumbnail(client.user.avatarURL())
         .setFooter(`Komut ${message.author.tag} Tarafından Kullanıldı ! `)
         db.set(`sunucu.ayarlar.bansistem.yetkili_${message.guild.id}`, rol.id)
  ...
  .setColor(client.ayarlar.embedRenk)
  .setDescription(`Ban Log Kanalı Belirt! `)
  .setThumbnail(client.user.avatarURL())
  .setFooter(`Komut ${message.author.tag} Tarafından Kullanıldı ! `)
  ...
  .setThumbnail(client.user.avatarURL())
  .setFooter(`Komut ${message.author.tag} Tarafından Kullanıldı ! `)
  db.set(`sunucu.ayarlar.bansistem.log_${message.guild.id}`, kanal.id)
  message.inlineReply(ayarlandı)
